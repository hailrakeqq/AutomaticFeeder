...
void HttpServer::begin(const char *ssid, const char *password,
  ...
  server.on("/feed",
            [this, stepperMotor]() { this->handleFeed(stepperMotor); });

  server.on("/change_feedamount", HTTP_POST,
            [this]() { this->handleChangeFeedAmount(); });

  server.on("/get_feedamount", [this]() { this->handleGetFeedAmount(); });
...
void HttpServer::handleRequest() { server.handleClient(); }
